...
static void mtk_spim_set_cs(struct mtk_spim_priv *priv, bool enable)
{
	u32 reg_val;
	...
	if (!enable) {
		...
		reg_val &= ~SPI_CMD_PAUSE_EN;
		writel(reg_val, priv->base + SPI_CMD_REG);
		priv->state = MTK_SPI_IDLE;
		mtk_spim_reset(priv);
	}
}

static int mtk_spim_hw_init(struct mtk_spim_priv *priv)
{
	u16 cpha, cpol;
	u32 reg_val;

	cpha = priv->mode & SPI_CPHA ? 1 : 0;
	cpol = priv->mode & SPI_CPOL ? 1 : 0;
...
static void mtk_spim_prepare_transfer(struct mtk_spim_priv *priv)
{
	...
	if (priv->speed_hz > 0 && priv->speed_hz <= spi_clk_hz / 4)
		div = DIV_ROUND_UP(spi_clk_hz, priv->speed_hz);
	else
		div = 4;
...
static int mtk_spim_transfer_wait(struct mtk_spim_priv *priv,
				  const struct spi_mem_op *op)
{
	int ret = 0;
	u32 reg;
	...
	us += 1000 * 1000 * 10; // TBD: add some margin
			    ...
			    const struct spi_mem_op *op)
{
	...
	priv->mode = slave->mode;
	priv->speed_hz = slave->max_hz;
#if !CONFIG_IS_ENABLED(DM_SPI)
	priv->cs = slave->cs;
#endif

	mtk_spim_reset(priv);
	mtk_spim_hw_init(priv);
	mtk_spim_prepare_transfer(priv);
	...
	ret = mtk_spim_transfer_wait(priv, op);
	if (ret)
		goto rx_unmap;
...
static int mtk_spim_get_attr(struct mtk_spim_priv *priv, struct udevice *dev)
{
	...
	if (ret < 0)
		dev_err(dev, "tick dly not set.\n");

	ret = dev_read_u32(dev, "sample_sel", &priv->sample_sel);
	if (ret < 0)
		dev_err(dev, "sample sel not set.\n");
...
static int mtk_spim_xfer(struct udevice *dev, unsigned int bitlen,
			 const void *dout, void *din, unsigned long flags)
{
	struct udevice *bus = dev_get_parent(dev);
	struct mtk_spim_priv *priv = dev_get_priv(bus);

	u32 reg_val, nio = 1, tx_size, packet_size, len;
	char *tx_tmp_buf;
	char *rx_tmp_buf;
	int ret = 0;
	u32 xfer_len = 0;
	dma_addr_t old_tx_dma;
	dma_addr_t old_rx_dma;
	...
	priv->xfer_len = bitlen / 8;
	mtk_spim_setup_packet(priv);

	tx_size = priv->xfer_len;
	tx_size = max(tx_size, (u32)32);

	if (priv->hw_cap.ipm_design)
		packet_size = MTK_SPI_IPM_PACKET_SIZE;
	else
		packet_size = MTK_SPI_PACKET_SIZE;
	...
	memcpy(tx_tmp_buf, dout, tx_size);
	...
	old_tx_dma = priv->tx_dma;

	if (!IS_ALIGNED((size_t)din, 4)) {
		rx_tmp_buf = kzalloc(tx_size, GFP_KERNEL);
		if (!rx_tmp_buf) {
			ret = -ENOMEM;
			goto tx_unmap;
		}
	...
	priv->rx_dma = dma_map_single(rx_tmp_buf,
				      tx_size, DMA_FROM_DEVICE);
	if (dma_mapping_error(priv->dev, priv->rx_dma)) {
		ret = -ENOMEM;
		goto rx_free;
	}
	old_rx_dma = priv->rx_dma;
	...
	mtk_spim_setup_dma_xfer(priv, NULL);
	mtk_spim_enable_transfer(priv);

	/* Wait for the interrupt. */
	ret = mtk_spim_transfer_wait(priv, NULL);
	if (ret)
		goto rx_unmap;

	if (priv->xfer_len > packet_size && (priv->xfer_len % packet_size != 0)) {
		xfer_len = priv->xfer_len;
		priv->state = MTK_SPI_PAUSED;
		len = priv->xfer_len - priv->xfer_len % packet_size;
		priv->xfer_len = priv->xfer_len % packet_size;
		mtk_spim_setup_packet(priv);

		priv->tx_dma = dma_map_single(tx_tmp_buf + len,
					      priv->xfer_len, DMA_TO_DEVICE);
		if (dma_mapping_error(priv->dev, priv->tx_dma)) {
			ret = -ENOMEM;
			xfer_len = 0;
			goto rx_unmap;
		...
		priv->rx_dma = dma_map_single(rx_tmp_buf + len,
					      priv->xfer_len, DMA_FROM_DEVICE);
		if (dma_mapping_error(priv->dev, priv->rx_dma)) {
			ret = -ENOMEM;
			reg_val = readl(priv->base + SPI_CMD_REG);
			reg_val &= ~SPI_CMD_TX_DMA;
...
rx_unmap:
	...
	if (xfer_len > packet_size)
		dma_unmap_single(priv->rx_dma,
				 priv->xfer_len, DMA_FROM_DEVICE);
rx_free_other:
	dma_unmap_single(old_rx_dma,
			 tx_size, DMA_FROM_DEVICE);
rx_free:
	if (!IS_ALIGNED((size_t)din, 4)) {
		kfree(rx_tmp_buf);
...
tx_unmap:
	if (xfer_len > packet_size)
		dma_unmap_single(priv->tx_dma,
				 priv->xfer_len, DMA_TO_DEVICE);
	dma_unmap_single(old_tx_dma,
			 tx_size, DMA_TO_DEVICE);
tx_free:
	kfree(tx_tmp_buf);
	tx_tmp_buf = NULL;
exit:
	mtk_spim_set_cs(priv, true);
	priv->use_spimem = false;
	return ret;
}
