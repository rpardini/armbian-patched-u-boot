...
static void mtk_delay_ps2stage(struct udevice *dev)
{
	struct eth_pdata *pdata = dev_get_plat(dev);
	struct eqos_mtk_priv *mtk_pdata = pdata->priv_pdata;
...
static void mtk_delay_stage2ps(struct udevice *dev)
{
	struct eth_pdata *pdata = dev_get_plat(dev);
	struct eqos_mtk_priv *mtk_pdata = pdata->priv_pdata;
...
static phy_interface_t eqos_get_interface_mtk(struct udevice *dev)
{
	const char *phy_mode;
...
static int eqos_probe(struct udevice *dev)
{
	 ...
	 */
	eqos->regs = dev_read_addr_index(dev, 0);
...
struct eqos_config __maybe_unused eqos_mtk_config = {
	.reg_access_always_ok = false,
	.mdio_wait = 10000,
	.swr_wait = 10,
	.config_mac = EQOS_MAC_RXQ_CTRL0_RXQ0EN_ENABLED_DCB,
