	...
	for (i = 0; i < MAX_ETHERNET; i++) {
		...
		if (ret <= 0 || !is_valid_ethaddr(&ethaddr[i * ARP_HLEN])) {
			if (!randomed) {
				...
				randomed = true;
                printf("%s: looping... generated RANDOMED... %d and ret %d\n", __func__, i, ret);
			} else {
				if (i > 0) {
...
static int rockchip_set_serialno(void)
{
	...
	int ret = 0, i;
	u64 serialno;
	printf("%s: starting serial number stuff\n", __func__);
	...
	if (!env_get("serial#")) {
        ...
        printf("%s: serial Generate the serial number based on CPU ID\n", __func__);
		for (i = 0; i < 8; i++) {
			low[i] = cpuid[1 + (i << 1)];
			high[i] = cpuid[i << 1];
		}
